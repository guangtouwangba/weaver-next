...
7. Immediately before writing, re-check `weaver_get_bound_canvas` and `weaver_get_agent_task`; stop without writing if the binding changed, went offline, or the task is not `running/content`. Then `weaver_submit_changeset`. The browser shows the ChangeSet preview over SSE.
