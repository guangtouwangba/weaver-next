...
export function applyLayoutOperations<T extends { layoutRevision: number; nodes: Record<string, MutableLayoutNode>; edges?: Record<string, MutableLayoutEdge>; groups?: Record<string, MutableLayoutGroup> }>(document: T, toRevision: number, operations: LayoutOperation[]): T {
  ...
  for (const operation of operations) {
    ...
    if (operation.type === "set-edge-route" && "edgeId" in operation) { next.edges ??= {}; const edges = next.edges; const edgeId = operation.edgeId as string; edges[edgeId] = { ...(edges[edgeId] ?? { edgeId }), ...(operation.route as Record<string, unknown>) }; }
    else if (operation.type === "create-group" && "groupId" in operation) { const op = operation as { groupId: string; frame: Record<string, unknown>; label?: string; kind?: string; direction?: string }; next.groups ??= {}; const groups = next.groups; groups[op.groupId] = { groupId: op.groupId, ...op.frame, kind: op.kind ?? "interaction", padding: 32, collapsed: false, ...(op.label !== undefined ? { label: op.label } : {}), ...(op.direction ? { direction: op.direction } : {}) }; }
    ...
    else if (operation.type === "set-group-frame" && "groupId" in operation) { const op = operation as { groupId: string; frame: Record<string, unknown> }; next.groups ??= {}; const groups = next.groups; groups[op.groupId] = { ...(groups[op.groupId] ?? { groupId: op.groupId }), ...op.frame }; }
