import { Crosshair, Focus, Group, Map as MapIcon, Minus, Moon, Plus, Sun, Trash2 } from "lucide-react";
...
export function CanvasNavigation(props: {
  ...
  onToggleMiniMap: () => void; onToggleTheme: () => void; onDeleteSelection: () => void; onGroupSelection: () => void; selectionCount: number;
}) {
  const { state, miniMapOpen, hasSelection, dark, onZoomOut, onZoomIn, onFit, onFocus, onToggleMiniMap, onToggleTheme, onDeleteSelection, onGroupSelection, selectionCount } = props;
  const { t } = useI18n();
  ...
  return <div className="canvas-navigation" aria-label={t("canvasNavigation")}>
    ...
    <button aria-label={t("groupSelection")} title={t("groupSelection")} disabled={!hasSelection} onClick={onGroupSelection}><Group size={14} /></button>
