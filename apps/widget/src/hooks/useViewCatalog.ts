...
export function useViewCatalog(params: {
  ...
  async function renameProjectView(viewId: string) {
    ...
    try { const output = await callTool<CatalogActionResult>("weaver_catalog_action", { action: "rename_view", mutationId: createMutationId(), workspaceDir: bootstrap.workspaceDir, projectId: project.id, viewId, name: renameValue.trim(), baseCatalogRevision: project.viewCatalogRevision, ...catalogLeaseArgs() }); applyCatalogResult(output); setRenamingViewId(null); setViewMenuId(null); setStatus(`Renamed View to ${output.view.name}`); }
    catch (error) { setStatus(error instanceof Error ? error.message : String(error)); }
  }

  async function pinProjectView(view: ProjectView) {
    if (!project) return;
    try { const output = await callTool<CatalogActionResult>("weaver_catalog_action", { action: "pin_view", mutationId: createMutationId(), workspaceDir: bootstrap.workspaceDir, projectId: project.id, viewId: view.id, pinned: !view.pinned, baseCatalogRevision: project.viewCatalogRevision, ...catalogLeaseArgs() }); applyCatalogResult(output); setViewMenuId(null); }
    catch (error) { setStatus(error instanceof Error ? error.message : String(error)); }
  }

  async function setDefaultView(view: ProjectView) {
    if (!project) return;
    try { const output = await callTool<CatalogActionResult>("weaver_catalog_action", { action: "set_default_view", mutationId: createMutationId(), workspaceDir: bootstrap.workspaceDir, projectId: project.id, viewId: view.id, baseCatalogRevision: project.viewCatalogRevision, ...catalogLeaseArgs() }); applyCatalogResult(output); setViewMenuId(null); setStatus(`${view.name} is now the default View`); }
    catch (error) { setStatus(error instanceof Error ? error.message : String(error)); }
  }
  ...
  async function reorderPinnedViews(targetViewId: string) {
    ...
    try { const output = await callTool<CatalogActionResult>("weaver_catalog_action", { action: "reorder_views", mutationId: createMutationId(), workspaceDir: bootstrap.workspaceDir, projectId: project.id, viewIds: ordered, baseCatalogRevision: project.viewCatalogRevision, ...catalogLeaseArgs() }); applyCatalogResult(output); }
    catch (error) { setStatus(error instanceof Error ? error.message : String(error)); }
  ...
  async function trashProjectView(view: ProjectView) {
    ...
    try {
      ...
      const output = await callTool<CatalogActionResult>("weaver_catalog_action", { action: "trash_view", mutationId: createMutationId(), workspaceDir: bootstrap.workspaceDir, projectId: project.id, viewId: view.id, fallbackViewId: fallback?.id, baseCatalogRevision: project.viewCatalogRevision, ...catalogLeaseArgs() });
  ...
  async function restoreProjectView(viewId: string) {
    const currentProject = projectRef.current; if (!currentProject) return;
    try { const output = await callTool<CatalogActionResult>("weaver_catalog_action", { action: "restore_view", mutationId: createMutationId(), workspaceDir: bootstrap.workspaceDir, projectId: currentProject.id, viewId, baseCatalogRevision: currentProject.viewCatalogRevision, ...catalogLeaseArgs() }); applyCatalogResult(output); setViewToast({ message: `Restored "${output.view.name}"` }); }
    catch (error) { setStatus(error instanceof Error ? error.message : String(error)); }
  }
  ...
  async function purgeProjectView(view: ProjectView) {
    ...
    try { const output = await callTool<CatalogActionResult>("weaver_catalog_action", { action: "purge_view", mutationId: createMutationId(), workspaceDir: bootstrap.workspaceDir, projectId: project.id, viewId: view.id, baseCatalogRevision: project.viewCatalogRevision, ...catalogLeaseArgs() }); applyCatalogResult(output); setPurgeConfirmId(null); setViewToast({ message: `Permanently deleted "${view.name}"` }); }
    catch (error) { setStatus(error instanceof Error ? error.message : String(error)); }
  }

  async function duplicateProjectView(view: ProjectView) {
    if (!project) return;
    try { const output = await callTool<CatalogActionResult>("weaver_catalog_action", { action: "duplicate_view", mutationId: createMutationId(), workspaceDir: bootstrap.workspaceDir, projectId: project.id, viewId: view.id, name: `${view.name} copy`, baseCatalogRevision: project.viewCatalogRevision, ...catalogLeaseArgs() }); applyCatalogResult(output); setViewMenuId(null); await switchView(output.view.id); }
    catch (error) { setStatus(error instanceof Error ? error.message : String(error)); }
  }
