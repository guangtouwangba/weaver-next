...
import type { Bootstrap, Candidate, ChatBindingBootstrap, ProjectView, VisualTemplate } from "./types";
...
function WeaverWidget() {
  ...
  const { bootstrap, setBootstrap, project, setProject, manifest, setManifest, layout, setLayout, graphNodes, setGraphNodes, graphEdges, setGraphEdges, status, setStatus, busy, setBusy, projectRef, layoutRef, graphNodesRef, graphEdgesRef, bindingRef, pendingInitialFitView, pendingViewportRestore, load, ensureBindingTarget, hydratePreviews } = bootstrapHook;

  const { syncContext } = useCanvasBindingSync({
    standaloneDemo, project, layout, bootstrap, bindingRef, setStatus, selection, nodes, viewport, sessionId, stream, pendingInitialFitView, pendingViewportRestore,
  });

  const { persistNodeFrame, persistNodeResize: _persistNodeResize, togglePinned, handleSelectionChange, handleNodeDrag, handleCanvasWheel } = useCanvasGraph({
    standaloneDemo, nodes, setNodes, setEdges, graphNodes, graphEdges, assetPreviews, draggingNodeId, viewport, selection, setSelection, bootstrap, project, layout, setLayout, layoutRef, projectRef, setStatus, load, pendingInitialFitView, pendingViewportRestore, fitView, getViewport, setViewport,
  });
  ...
  return <main className="weaver-shell" data-editor-open={Boolean(activeDocument)}>
    ...
    <section className="workspace-stage">
      ...
      />
      <CanvasStage
        standaloneDemo={standaloneDemo} displayedNodes={displayedNodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} layout={layout} handleCanvasWheel={handleCanvasWheel} handleNodeDrag={handleNodeDrag} handleSelectionChange={handleSelectionChange}
