...
describe("follow-up tasks", () => {
  it("captures one branch anchor and keeps the other selected nodes as explicit references", () => {
    ...
    const project = db.catalog.createProject({ title: "Follow up", goal: "", scenePack: scene });
    ...
    db.sessions.syncCanvas({ version: 2, canvasSessionId: "follow-up-canvas", workspaceDir: root, projectId: project.id, scenePackId: scene.id, scenePackVersion: scene.version, graphRevision: 0, viewId: project.defaultViewId, viewType: scene.defaultView, focusedNodeId: "node-b", selectedNodeIds: ["node-a", "node-b", "node-c"], selectedEdgeIds: [], selectedGroupIds: [], pinnedContextNodeIds: [], viewport: { x: 0, y: 0, zoom: 1 }, presence: { visible: true, focused: true, lastSeenAt: timestamp }, chatBinding: { leaseId: binding.leaseId, bindingRevision: binding.bindingRevision }, agentEligible: true, sequence: 1, updatedAt: timestamp }, chatSessionKey);
    const task = db.tasks.prepare({ canvasSessionId: "follow-up-canvas", actionKey: "follow_up_ask", userInstruction: "Compare these ideas", dispatchKey: "follow-up-1", chatSessionKey });
