...
export class WorkspaceStore {
  ...
  readonly assets = { get: (assetId: string) => assets.getAsset(this.db, assetId), byHash: (projectId: string, sha256: string) => assets.getAssetByHash(this.db, projectId, sha256), read: (assetId: string, thumbnail = false) => assets.readAsset(this.db, this.dataDir, assetId, thumbnail), importImage: (input: { projectId: string; mimeType: Asset["mimeType"]; data: Uint8Array }) => assets.importImageAsset(this.db, this.dataDir, input), deleteUnreferenced: (assetId: string) => assets.deleteUnreferencedAsset(this.db, this.dataDir, assetId), saveTaskFile: (taskId: string, fileName: string, data: Uint8Array) => assets.saveTaskAsset(this.dataDir, taskId, fileName, data) };
