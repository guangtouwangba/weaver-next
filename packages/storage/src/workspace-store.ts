...
export class WorkspaceStore {
  ...
  readonly graphChanges = {
    read: (projectId: string): GraphSnapshot => graph.getGraph(this.db, projectId), replace: (snapshot: GraphSnapshot, eventContext: { taskId?: string; canvasSessionId?: string } = {}) => graph.replaceGraph(this.db, snapshot, eventContext), createNode: (input: { projectId: string; viewId: string; type: string; title: string; content: NodeContent; x: number; y: number }) => graph.createContentNode(this.db, input), updateNode: (input: { projectId: string; nodeId: string; baseGraphRevision: number; title?: string; type?: string; content?: NodeContent }) => graph.updateNodeContent(this.db, input), archiveNode: (input: { projectId: string; nodeId: string; baseGraphRevision: number }) => graph.archiveNode(this.db, input), attachAsset: (input: { projectId: string; nodeId: string; assetId: string; role: "embedded" | "cover"; baseGraphRevision: number }) => graph.attachAsset(this.db, input),
