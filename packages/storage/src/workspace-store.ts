...
export class WorkspaceStore {
  ...
  readonly catalog = {
    ...
    deletePristineProject: (input: { projectId: string; creationMutationId: string }) => projects.deletePristineProject(this.db, input),
    ...
    restoreSnapshot: (input: { projectId: string; defaultViewId: string; views: ProjectView[]; layouts: LayoutDocument[] }) => viewCatalog.restoreProjectCatalogSnapshot(this.db, input),
  ...
  readonly assets = { get: (assetId: string) => assets.getAsset(this.db, assetId), byHash: (projectId: string, sha256: string) => assets.getAssetByHash(this.db, projectId, sha256), read: (assetId: string, thumbnail = false) => assets.readAsset(this.db, this.dataDir, assetId, thumbnail), prepareImage: (input: { projectId: string; mimeType: Asset["mimeType"]; data: Uint8Array }) => assets.prepareImageAsset(this.db, input), commitPreparedImage: (prepared: assets.PreparedImageAsset) => assets.commitPreparedImageAsset(this.db, this.dataDir, prepared), importImage: (input: { projectId: string; mimeType: Asset["mimeType"]; data: Uint8Array }) => assets.importImageAsset(this.db, this.dataDir, input), deleteUnreferenced: (assetId: string) => assets.deleteUnreferencedAsset(this.db, this.dataDir, assetId), saveTaskFile: (taskId: string, fileName: string, data: Uint8Array) => assets.saveTaskAsset(this.dataDir, taskId, fileName, data) };
  ...
  readonly browserSessions = {
    ...
    create: (input: { id: string; credentialHash: string; now: string; expiresAt: string; projectId?: string; viewId?: string }) => browserSessions.createBrowserSession(this.db, input),
    setTarget: (input: { id: string; projectId?: string; viewId?: string; now: string }) => browserSessions.setBrowserSessionTarget(this.db, input),
