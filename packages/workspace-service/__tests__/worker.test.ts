...
describe("workspace worker", () => {
  ...
  it("opens a duplicate Project session read-only until explicit takeover", async () => {
    ...
    expect(await fetch(`${origin}/api/bootstrap`, { headers: { cookie: first } }).then((response) => response.json())).toMatchObject({ capabilities: { manualWrite: false, canTakeOver: true } });
