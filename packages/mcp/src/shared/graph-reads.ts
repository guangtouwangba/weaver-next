 ...
 * tools (`weaver_read_graph(resource:"manifest")`, `weaver_read_graph(resource:"full")`,
 * `weaver_read_graph(resource:"node")`) and the model-facing `weaver_read_graph`, so the
...
/** weaver_read_graph(resource:"manifest") core. */
export function readProjectManifest(store: WorkspaceStore, projectId: string) {
  const project = store.catalog.getProject(projectId); if (!project) throw new Error("PROJECT_NOT_FOUND");
...
/** weaver_read_graph(resource:"full") core. */
export function readProjectGraph(store: WorkspaceStore, projectId: string, viewId?: string) {
  ...
  const layout = store.layoutReviews.get(projectId, viewId ?? project.defaultViewId); if (!layout) throw new Error("LAYOUT_NOT_FOUND");
...
/** weaver_read_graph(resource:"node") core. */
