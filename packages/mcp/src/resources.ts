...
import { getScenePack, builtinScenePacks } from "@weaver/scene-packs";
...
export function registerResources(server: McpServer, ctx: ResourcesCtx) {
  ...
  server.registerResource("weaver-agent-task", new ResourceTemplate("weaver://agent-tasks/{taskId}", { list: undefined }), { title: "Weaver Agent Task", description: "One persisted AgentTask. Call a task tool first so the server can resolve its workspace.", mimeType: "application/json" }, async (uri, variables) => { const taskId = String(variables.taskId); const workspaceDir = workspaceByTask.get(taskId); if (!workspaceDir) throw new Error("WORKSPACE_UNKNOWN_CALL_WEAVER_GET_AGENT_TASK_FIRST"); const task = withStore(workspaceDir, (store) => store.tasks.get(taskId)); if (!task) throw new Error("AGENT_TASK_NOT_FOUND"); return { contents: [{ uri: uri.href, mimeType: "application/json", text: JSON.stringify(task) }] }; });
  server.registerResource("weaver-project-views", new ResourceTemplate("weaver://projects/{projectId}/views", { list: undefined }), { title: "Weaver Project Views", description: "Durable saved View catalog records for a project, including recycle-bin state.", mimeType: "application/json" }, async (uri, variables) => { const projectId = String(variables.projectId); const workspaceDir = workspaceByProject.get(projectId); if (!workspaceDir) throw new Error("WORKSPACE_UNKNOWN_CALL_A_WEAVER_TOOL_FIRST"); const views = withStore(workspaceDir, (store) => store.catalog.listViews(projectId)); return { contents: [{ uri: uri.href, mimeType: "application/json", text: JSON.stringify(views) }] }; });
  server.registerResource("weaver-view-projection", new ResourceTemplate("weaver://projects/{projectId}/views/{viewId}/projection", { list: undefined }), { title: "Weaver View Projection", description: "Projection and theme metadata for one visual view.", mimeType: "application/json" }, async (uri, variables) => { const projectId = String(variables.projectId); const workspaceDir = workspaceByProject.get(projectId); if (!workspaceDir) throw new Error("WORKSPACE_UNKNOWN_CALL_A_WEAVER_TOOL_FIRST"); const layout = withStore(workspaceDir, (store) => store.layoutReviews.get(projectId, String(variables.viewId))); if (!layout) throw new Error("LAYOUT_NOT_FOUND"); return { contents: [{ uri: uri.href, mimeType: "application/json", text: JSON.stringify({ viewId: layout.viewId, viewName: layout.viewName, viewType: layout.viewType, templateRef: layout.templateRef, projection: layout.projection, theme: layout.theme }) }] }; });
