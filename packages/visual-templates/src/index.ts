...
function theme(family: VisualFamily): ViewTheme {
  ...
  return { ...structuredClone(defaultViewTheme), canvas: { backgroundColor: background, pattern: family === "table" ? "grid" : "dots", patternColor: `${accent}33` }, nodeStyles: { default: { fill, borderColor: `${accent}88`, textColor: "#20231f", accentColor: accent, borderRadius: family === "flow" ? 5 : 10, titleScale: family === "hierarchy" ? 1.12 : 1 } }, edgeStyles: { default: { color: accent, width: 1.6, dashed: family === "relationship", routing: family === "flow" ? "orthogonal" : "bezier", marker: "arrow" } } };
}
