...
export const widgetLayoutEdgeSchema = z.object({ edgeId: z.string(), routing: z.enum(["straight", "bezier", "orthogonal", "bundled"]), sourcePort: z.string().optional(), targetPort: z.string().optional(), waypoints: z.array(z.object({ x: z.number(), y: z.number() })), hidden: z.boolean().optional() }).passthrough();
export const widgetLayoutGroupSchema = z.object({ groupId: z.string(), x: z.number(), y: z.number(), width: z.number(), height: z.number() }).passthrough();
