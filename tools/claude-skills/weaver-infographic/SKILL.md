...
2. Gather the source content.
   - Read the selection: `weaver_read_session(resource:"canvas_context", canvasSessionId)` and `weaver_read_session(resource:"resolved_context", canvasSessionId)`.
...
6. Ingest: `weaver_import_asset(source:"bytes")` with `{ projectId, mimeType, base64 }` → `assetId`.
