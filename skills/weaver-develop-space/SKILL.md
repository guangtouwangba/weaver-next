...
6. Treat graph results as summaries. Call `weaver_read_graph(resource:"node")` only for nodes whose complete Markdown or media references are necessary, and use `weaver_get_asset_metadata` before requesting original image resources.
